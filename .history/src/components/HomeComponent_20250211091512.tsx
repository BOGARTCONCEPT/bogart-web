  ...
  return (
    ...
    >
      ...
      <div className="absolute right-10 top-1/2 transform -translate-y-1/2 text-right">
        {["EVENT", "PRODUCTS", "RENDERS", "HISTORY"].map((item, index) => (
          <Link
            key={index}
            href={`/${item.toLowerCase()}`}
            className="block mb-4 text-4xl font-bold text-orange-600 hover:text-gray-700 transition-colors duration-300 ease-in-out"
          >
            {item}
          </Link>
        ))}
