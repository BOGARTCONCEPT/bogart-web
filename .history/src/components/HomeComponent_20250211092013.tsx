    ...
    >
      ...
      <div className="absolute right-10 top-1/2 transform -translate-y-1/2 text-right">
        {menuItems.map((item, index) => (
          item.enabled ? ( // Conditionally render Link if enabled is true
            <Link
              key={index}
              href={item.url}
          ...
          ) : ( // Render a disabled span if enabled is false
            <span
              key={index}
              className="block mb-4 text-4xl font-bold text-gray-400 cursor-default" // Visually indicate disabled state
            >
              {item.text}
            </span>
          )
