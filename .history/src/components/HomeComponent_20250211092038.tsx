    ...
    >
      ...
      <div className="absolute right-10 top-1/2 transform -translate-y-1/2 text-right">
        {menuItems.map((item, index) => (
          <React.Fragment key={index}> {/* Key is now on a Fragment */}
            {item.enabled ? (
              <Link
                href={item.url}
              ...
              <span className="group block mb-4 text-4xl font-bold text-orange-600 transition-all duration-300 ease-in-out cursor-default">
                <span className="inline-block transform transition-all duration-300 ease-in-out">
                  {item.text}
                </span>
              </span>
            )}
